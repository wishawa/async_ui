...
use crate::{
    create_portal, element::Element, executor::GtkSpawner, manual_apis::GtkBackend, PortalExit,
};

pub async fn list<'a, K: Eq + Hash + Clone>(children: &Rx<Vec<(K, Option<Element<'a>>)>>) {
    struct ChildTask {
        exit_portal: PortalExit,
        exit_task: Option<<GtkSpawner as Spawner>::Task>,
        _entry_task: <GtkSpawner as Spawner>::Task,
        index: usize,
    }
    let parent_control = GtkBackend::get_tls().with(Clone::clone);
